...
function Navbar() {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-dark py-2" style={{ backgroundColor: "rgb(0, 0, 0)" }}>
      <div className="container-fluid px-4">
        <a className="navbar-brand me-auto" href="/">
          <img src="/images/GNTT.jpg" alt="Company Logo" style={{ height: "50px", objectFit: "contain", margin: "0" }} />
        </a>
          ...
          <ul className="navbar-nav d-flex align-items-center gap-3">
            {topics.map((topic) => (
              <li className="nav-item" key={topic.id}>
                <button className="nav-link btn btn-link" onClick={() => handleTopicClick(topic.id)}>
                  {topic.vi}
                </button>
              </li>
            ))}
